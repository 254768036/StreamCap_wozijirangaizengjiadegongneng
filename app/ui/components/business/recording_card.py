...
class RecordingCardManager:
    def __init__(self, app):
        ...
        self.selected_cards = {}
        self.video_player = VideoPlayer(app)
    ...
    def _create_card_components(self, recording: Recording):
        ...
        duration_text_label = ft.Text(self.app.record_manager.get_duration(recording), size=10)
        ...
        speed_text_label = ft.Text(speed, size=10)
    ...
    async def preview_video_button_on_click(self, _, recording: Recording):
        if recording.record_url:
            await self.video_player.preview_video(
                source=recording.record_url, is_file_path=False, room_url=recording.url
            )
        elif recording.preview_url:
            await self.video_player.preview_video(
                source=recording.preview_url, is_file_path=False, room_url=recording.url
            )
        elif recording.url:
            self.app.page.launch_url(recording.url)
            await self.app.snack_bar.show_snack_bar(f"{self._['preview_video']}: 已在浏览器中打开直播间")
        else:
            await self.app.snack_bar.show_snack_bar("直播间URL无效或为空")
