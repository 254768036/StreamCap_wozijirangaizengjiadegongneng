...
from .stream_manager import tiered_cache, adaptive_scheduler, metrics_collector
...
class RecordingManager:
    ...
    async def stop_monitor_recording(self, recording: Recording, auto_save: bool = True):
        ...
        if recording.monitor_status:
            ...
            self.app.page.run_task(self.app.record_card_manager.update_card, recording)
            self.app.pubsub.send_others_on_topic("update", recording)
            if auto_save:
                self.app.page.run_task(self.persist_recordings)
        ...
        for i, recording in enumerate(self.recordings):
            if recording.monitor_status and not recording.is_recording:
                ...
                if not recording.detection_time or is_exceeded:
                    recordings_to_check.append((i, recording))

        if not recordings_to_check:
            return

        logger.info(f"Batch check live status: {len(recordings_to_check)} recordings")

        for idx, recording in recordings_to_check:
            if not self.app.recording_enabled:
                break

            delay = adaptive_scheduler.get_start_delay(idx, len(recordings_to_check), batch_size=5)
            if delay > 0:
                await asyncio.sleep(delay)

            poll_interval = tiered_cache.get_poll_interval(recording.url)
            self.app.page.run_task(self.check_if_live, recording, poll_interval)
    ...
    async def check_if_live(self, recording: Recording, poll_interval: int = None):
        """检查直播状态，支持动态轮询间隔"""
        ...
        if poll_interval:
            recording.loop_time_seconds = poll_interval
