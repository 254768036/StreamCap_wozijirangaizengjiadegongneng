...
def _safe_destroy_window(page):
    try:
        ...
        try:
            if hasattr(page, "data") and hasattr(page.data, "record_manager"):
                for recording in page.data.record_manager.recordings:
                    if hasattr(recording, "force_stop"):
                        recording.force_stop = True
        except Exception as e:
            logger.debug(f"Failed to set force_stop for recordings: {e}")

        try:
            if hasattr(page, "data") and hasattr(page.data, "cleanup"):
                asyncio.create_task(page.data.cleanup())
        except Exception as e:
            logger.debug(f"Failed to cleanup app: {e}")

        if hasattr(page, "data") and hasattr(page.data, "tray_manager"):
            if not getattr(page.data, "is_web_mode", False):
                page.data.tray_manager.stop()
        page.window.destroy()
                ...
                try:
                    ...
                    try:
                        if hasattr(app, "record_manager"):
                            for recording in app.record_manager.recordings:
                                if hasattr(recording, "force_stop"):
                                    recording.force_stop = True
                    except Exception as e:
                        logger.debug(f"Failed to set force_stop for recordings: {e}")
